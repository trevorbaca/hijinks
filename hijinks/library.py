...
order_1 = abjad.Sequence(cary).flatten()
order_1 = [_ % 12 for _ in order_1]
assert order_1 == [10, 0, 2, 6, 8, 7, 5, 3, 1, 9, 4, 11]

order_2 = [abjad.Sequence(_).rotate(n=1) for _ in cary]
order_2 = abjad.Sequence(order_2).rotate(n=-1)
order_2 = abjad.Sequence(order_2).flatten()
order_2 = [_ % 12 for _ in order_2]
assert order_2 == [5, 6, 8, 7, 11, 3, 1, 9, 4, 2, 10, 0]

order_3 = [abjad.Sequence(_).rotate(n=2) for _ in cary]
order_3 = abjad.Sequence(order_3).rotate(n=-2)
order_3 = abjad.Sequence(order_3).flatten()
